...
class RudderUserPasswordEncoderTest extends Specification {
  ...
  "hash algo recognition" should {
    ...
    "be ok for md5" in {
      RudderPasswordEncoder.getFromEncoded(pass1_md5, legacy) must beRight(RudderPasswordEncoder.MD5)
    }
    "be ok for sha1" in {
      RudderPasswordEncoder.getFromEncoded(pass1_sha1, legacy) must beRight(RudderPasswordEncoder.SHA1)
    }
    "be ok for sha256" in {
      RudderPasswordEncoder.getFromEncoded(pass1_sha256, legacy) must beRight(RudderPasswordEncoder.SHA256)
    }
    "be ok for sha512" in {
      RudderPasswordEncoder.getFromEncoded(pass1_sha512, legacy) must beRight(RudderPasswordEncoder.SHA512)
    }
    "be ok for bcrypt a" in {
      RudderPasswordEncoder.getFromEncoded(pass1_bcrypt_a, legacy) must beRight(RudderPasswordEncoder.BCRYPT)
    }
    "be ok for bcrypt y" in {
      RudderPasswordEncoder.getFromEncoded(pass1_bcrypt_y, legacy) must beRight(RudderPasswordEncoder.BCRYPT)
    }
    ...
    "allow safe bcrypt hash in modern mode" in {
      RudderPasswordEncoder.getFromEncoded(pass1_bcrypt_y, modern) must beRight(RudderPasswordEncoder.BCRYPT)
    }
  }
